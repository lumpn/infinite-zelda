...
{
    public sealed class NotEqualsGene : ZeldaGene
    {
        ...
        public NotEqualsGene(ZeldaConfiguration configuration, string variableName, string blockerName, int targetValue)
            : base(configuration)
        {
        ...
        {
            return new NotEqualsGene(configuration, variableName, blockerName, targetValue);
        }

        public override void Express(CrawlerBuilder builder, VariableLookup lookup)
        {
            var script = new NotEqualsScript(targetValue, blockerName, variableName, lookup);
